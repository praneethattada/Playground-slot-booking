import React, { useState, useEffect, useCallback } from 'react';
import { NavLink, useNavigate } from 'react-router-dom';
import './Navbar.css';

const Navbar = () => {
  const navigate = useNavigate();
  
  // State to track login status
  const [auth, setAuth] = useState({ 
    user: !!localStorage.getItem('authToken'), 
    admin: !!localStorage.getItem('adminAuthToken') 
  });
  
  const [isScrolled, setIsScrolled] = useState(false);
  const [isMenuOpen, setIsMenuOpen] = useState(false);

  // --- CORE FIX: This function checks localStorage and updates the component's state ---
  const checkAuthStatus = useCallback(() => {
    const userToken = !!localStorage.getItem('authToken');
    const adminToken = !!localStorage.getItem('adminAuthToken');
    setAuth({ user: userToken, admin: adminToken });
  }, []);

  // This effect adds event listeners to automatically update the navbar
  useEffect(() => {
    // Listen for the 'storage' event, which fires when localStorage changes in another tab/window
    window.addEventListener('storage', checkAuthStatus);
    
    // Also listen for a custom 'authChange' event for same-page updates
    window.addEventListener('authChange', checkAuthStatus);

    // Initial check when the component mounts
    checkAuthStatus();

    // Cleanup listeners on component unmount
    return () => {
      window.removeEventListener('storage', checkAuthStatus);
      window.removeEventListener('authChange', checkAuthStatus);
    };
  }, [checkAuthStatus]);

  // Effect for the scroll background change
  useEffect(() => {
    const handleScroll = () => setIsScrolled(window.scrollY > 50);
    window.addEventListener('scroll', handleScroll);
    return () => window.removeEventListener('scroll', handleScroll);
  }, []);

  const handleLogout = () => {
    localStorage.clear();
    // Dispatch the custom event so all components (like this navbar) know about the change
    window.dispatchEvent(new Event('authChange'));
    closeMobileMenu();
    navigate('/');
  };

  const closeMobileMenu = () => setIsMenuOpen(false);

  return (
    <nav className={`navbar-glass ${isScrolled || isMenuOpen ? 'active' : ''}`}>
      <div className="navbar-container">
        <NavLink to="/" className="navbar-logo" onClick={closeMobileMenu}>
          {/* SVG Logo */}
          <svg id="logo-16" width="109" height="43" viewBox="0 0 109 43" fill="none" xmlns="http://www.w3.org/2000/svg"> <path d="M64.9315 11.4284C62.1883 8.6852 58.9316 6.5091 55.3475 5.0245C51.7633 3.5399 47.9219 2.7758 44.0424 2.7758C40.1629 2.7758 36.3215 3.5399 32.7373 5.0245C29.1532 6.5091 25.8965 8.6852 23.1533 11.4284L44.0424 32.3174L64.9315 11.4284Z" class="ccompli1" fill="#FFD200"></path> <path d="M44.0686 32.3475C46.8118 35.0907 50.0684 37.2667 53.6526 38.7513C57.2367 40.2359 61.0782 41 64.9577 41C68.837 41 72.679 40.2359 76.263 38.7513C79.847 37.2667 83.104 35.0907 85.847 32.3475L64.9577 11.4584L44.0686 32.3475Z" class="ccompli2" fill="#06E07F"></path> <path d="M44.017 32.3429C41.2738 35.0861 38.0171 37.2621 34.433 38.7467C30.8488 40.2313 27.0074 40.9954 23.1279 40.9954C19.2484 40.9954 15.407 40.2313 11.8228 38.7467C8.2387 37.2621 4.982 35.0861 2.2388 32.3429L23.1279 11.4538L44.017 32.3429Z" class="ccustom" fill="#E3073C"></path> <path d="M64.9831 11.433C67.726 8.6898 70.983 6.5138 74.567 5.0292C78.151 3.5446 81.993 2.7805 85.872 2.7805C89.752 2.7805 93.593 3.5446 97.177 5.0292C100.761 6.5138 104.018 8.6898 106.761 11.433L85.872 32.3221L64.9831 11.433Z" class="ccustom" fill="#1F84EF"></path> </svg>
        </NavLink>

        <div className="menu-icon" onClick={() => setIsMenuOpen(!isMenuOpen)}>
          {isMenuOpen ? '✕' : '☰'}
        </div>

        <ul className={`nav-menu ${isMenuOpen ? 'active' : ''}`}>
          <li className="nav-item">
            <NavLink to="/" className="nav-link" onClick={closeMobileMenu}>Home</NavLink>
          </li>

          {/* Show user links only if a user is logged in AND an admin is NOT logged in */}
          {auth.user && !auth.admin && (
            <>
              <li className="nav-item">
                <NavLink to="/check" className="nav-link" onClick={closeMobileMenu}>Book Slot</NavLink>
              </li>
              <li className="nav-item">
                <NavLink to="/my-bookings" className="nav-link" onClick={closeMobileMenu}>My Bookings</NavLink>
              </li>
            </>
          )}

          {auth.admin && (
            <li className="nav-item">
              <NavLink to="/adminview" className="nav-link" onClick={closeMobileMenu}>Dashboard</NavLink>
            </li>
          )}

          <li className="nav-item nav-buttons">
            {auth.user || auth.admin ? (
              <button onClick={handleLogout} className="btn btn-logout">Logout</button>
            ) : (
              <>
                <NavLink to="/login" className="btn btn-login" onClick={closeMobileMenu}>User Login</NavLink>
                <NavLink to="/admin" className="btn btn-admin" onClick={closeMobileMenu}>Admin</NavLink>
              </>
            )}
          </li>
        </ul>
      </div>
    </nav>
  );
};

export default Navbar;
